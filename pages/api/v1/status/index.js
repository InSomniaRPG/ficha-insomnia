...
async function status(request, response) {
  ...
  const databaseMaxConn = await database.query("SELECT setting::int FROM pg_settings WHERE name = 'max_connections';")
  const databaseActiveConn = await database.query("SELECT * FROM pg_stat_activity;")
